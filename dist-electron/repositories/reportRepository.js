...
function getIncomeReport(startDate, endDate) {
    const report = db.drizzle.select({
        ...
        .where(and(gte(sql `date(${payments.payment_date}, 'localtime')`, sql `date(${startDate})`), lte(sql `date(${payments.payment_date}, 'localtime')`, sql `date(${endDate})`))).get();
...
function getCompletedServices(startDate, endDate) {
    return db.drizzle.select({
        ...
        .where(and(like(serviceOrders.service_status, '%Selesai%'), gte(sql `date(${serviceOrders.completed_date}, 'localtime')`, sql `date(${startDate})`), lte(sql `date(${serviceOrders.completed_date}, 'localtime')`, sql `date(${endDate})`))).all();
}
function getTopSpareparts(startDate, endDate) {
    return db.drizzle.select({
        ...
        .where(and(like(serviceOrders.service_status, '%Selesai%'), gte(sql `date(${serviceOrders.completed_date}, 'localtime')`, sql `date(${startDate})`), lte(sql `date(${serviceOrders.completed_date}, 'localtime')`, sql `date(${endDate})`)))
