...
function getDashboardStats() {
    ...
    const inProgressQuery = db.drizzle.select({ count: sql `COUNT(*)` })
        .from(serviceOrders)
        .where(and(notLike(serviceOrders.service_status, '%Selesai%'), notInArray(serviceOrders.service_status, ['Batal', 'Dibatalkan'])))
        .get();
    ...
    const waitingQuery = db.drizzle.select({
        ...
        .where(and(eq(serviceOrders.service_status, 'Menunggu Sparepart'), sql `(julianday('now', 'localtime') - julianday(${serviceOrders.created_at}, 'localtime')) > 7`)).all();
    ...
    const completedNotPickedQuery = db.drizzle.select({
        ...
        .innerJoin(customers, eq(serviceOrders.customer_id, customers.id))
        .where(and(eq(serviceOrders.service_status, 'Selesai (Belum Diambil)'), isNotNull(serviceOrders.completed_date), sql `(julianday('now', 'localtime') - julianday(${serviceOrders.completed_date}, 'localtime')) > 14`)).all();
    ...
    const deadlineQuery = db.drizzle.select({
        ...
        .where(and(isNotNull(serviceOrders.estimated_completion_date), lte(sql `date(${serviceOrders.estimated_completion_date}, 'localtime')`, today), notLike(serviceOrders.service_status, '%Selesai%'), notInArray(serviceOrders.service_status, ['Batal', 'Dibatalkan']))).all();
    ...
    const waitingPartQuery = db.drizzle.select({
        ...
        .where(eq(serviceOrders.service_status, 'Menunggu Sparepart')).all();
    const todoItems = [
        ...deadlineQuery,
        ...waitingPartQuery
    ];
